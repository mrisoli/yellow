...
/** GET /integrations/salesforce/callback — Salesforce OAuth callback. */
http.route({
  ...
  handler: httpAction(async (ctx, request) => {
    ...
    if (error) {
      return new Response(
        JSON.stringify({ error: `Salesforce auth failed: ${error}` }),
        {
          status: 400,
    ...
    try {
      await ctx.runMutation(internal.salesforce.connectSalesforce, {
        code,
        redirectUri,
      });

      return new Response(
        JSON.stringify({ success: true, message: "Connected to Salesforce" }),
        {
          status: 200,
          headers: { "Content-Type": "application/json" },
        }
      ...
      return new Response(
        JSON.stringify({ error: `Salesforce connection failed: ${message}` }),
        {
          status: 500,
          headers: { "Content-Type": "application/json" },
        }
